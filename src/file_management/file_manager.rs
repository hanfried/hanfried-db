...
mod tests {
    ...
    const TEST_FILES_MAX: usize = 1000;
    const TEST_FILES_BLOCKS: usize = 50;
    const TEST_FILES_BLOCKSIZE: usize = 4096;
    const TEST_FILES_DB_DIRECTORY: &str = "/data/hanfried-db-unittest";
    #[test]
    fn test_file_manager() {
        let file_manager = FileManager::new(TEST_FILES_DB_DIRECTORY, TEST_FILES_BLOCKSIZE).unwrap();
                ...
                parallel_read_threads.push(thread::spawn(move || {
                    ...
                    loop {
                        ...
                        if file_nr_got == file_nr as i32 && block_nr_got == block_nr as i32 {
                            ...
                            // eprintln!("file_nr={}, block_nr={} not yet in sync (shows {},{})", file_nr, block_nr, file_nr_got, block_nr_got)
                        }
                    }
                }));
            }
        ...
        assert_eq!(TEST_FILES_MAX, file_manager.open_files_count());
    }
}
